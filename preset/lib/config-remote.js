/**
 * anchor-seed host-side Typert Remote bridge.
 *
 * The browser floating panel reads and writes the panel settings file through
 * the `anchorSeedConfig.get/set` endpoints. Like dsh-read-image, this exists
...
export function getAnchorSeedConfigService() {
 ...
 */
export function createAnchorSeedConfigBridge(protocol, store) {
  const { TypertRemoteService } = protocol
  return class AnchorSeedConfigService extends TypertRemoteService {
    constructor(ctx) {
      super(ctx, 'anchorSeedConfig')
