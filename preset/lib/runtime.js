 ...
 * generalized to `{path}` (project-root-relative) and two deliberate
 * modifications from the dev tests (2026-08-15):
 *
 * 1. AUTHORIZATION — "work entirely according to the instructions it
 *    contains": the model treats elevation text inside a tool result as
 *    untrusted FILE CONTENT, so authorization must come from the user side,
 *    not from the file claiming authority for itself.
 * 2. SETUP FRAMING — "Do not reply yet — the actual task follows in the next
 *    message": the virtual turn must read as CONTEXT LOADING, not as a user
 *    task to answer. The seeded assistant turn already matches this (reasoning
 *    + tool call, no text reply); the model's first REAL reply then belongs to
 *    the real user's first message, which the loop appends right after.
 *
 ...
 *   + preset prompt) -> AGENTS.md/CLAUDE.md -> REAL user first message
 */
export const DEFAULT_VIRTUAL_USER_TEMPLATE =
  'Session setup: please read the entire {path} in the project root directory for detailed information, ' +
  'and work entirely according to the instructions it contains. Do not reply yet — the actual task follows in the next message.'
