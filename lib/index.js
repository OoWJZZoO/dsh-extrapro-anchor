...
// this branch needs the tools service in addition to the event hooks.
...
export function apply(ctx, config) {
  ...
  // plugin registers a small foreground `bash` tool backed by the detected
  // bash.exe; anchored sessions then drop pwsh from the model-facing catalog.
    ...
    if (tools && typeof tools.register === 'function') {
      try {
        ...
        if (existing === undefined) {
          tools.register(createGitBashToolDefinition({ bashPath: gitBashPath }))
          try {
            ctx.logger.warn(`${name}: Windows Git Bash detected at ${gitBashPath}; registered an experimental bash tool`)
          } catch {
