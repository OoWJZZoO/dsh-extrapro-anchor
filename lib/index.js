...
import { createAnchorSeedConfigBridge } from './config-remote.js'
...
export function parseConfig(config) {
  ...
  // OFF (the panel's requested out-of-the-box posture). The self-contained
  // preset sets `enabled: true` because it ships no panel to turn it on.
 ...
 *
 * The self-contained preset copies `lib/` into `$DSH_HOME/.agent-presets/…`,
 * where `@deepseek-ai/dsh-typert-protocol` is NOT resolvable — a top-level
 * import would take the whole preset boot down. `createRequire` lets us try
 * synchronously (Node 22 require-ESM) and degrade to "panel config bridge
 * unavailable" (injection itself still works) instead of failing to load.
 */
...
export function apply(ctx, config) {
  ...
  // then calls anchorSeedConfig.set; this store re-reads the file on mtime
  ...
  // typert (self-contained preset) or without ctx.plugin (unusual host), the
  // panel cannot edit settings, but injection still works from row config.
  const typertProtocol = loadTypertProtocol()
  if (typertProtocol !== null && typeof ctx.plugin === 'function') {
    try {
      ctx.plugin(createAnchorSeedConfigBridge(typertProtocol, settingsStore))
    } catch (error) {
      try {
