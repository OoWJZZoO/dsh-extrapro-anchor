...
export function apply(ctx, config) {
  ...
  // edit settings, but injection still works from row config. The bridge also
  // carries the Windows Git Bash detection facts captured above, so the panel
  // can replace the generic "let me" drift warning with the actionable
  // "Git Bash 未安装" diagnosis and its install-guide link.
  ...
  if (typertProtocol !== null && typeof ctx.plugin === 'function') {
    try {
      ctx.plugin(createExtraproAnchorConfigBridge(typertProtocol, settingsStore, {
        ...
        gitBashInstalled: Boolean(gitBashPath),
